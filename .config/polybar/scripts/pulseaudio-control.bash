...
MUTED_COLOR="%{F#ff217c}"  # Color when the audio is muted
UNMUTED_COLOR="%{F#ffffff}"  # Color when the audio is unmuted
NOTIFICATIONS="no"  # Notifications when switching sinks if enabled
SINK_ICON="󰋋 "  # Icon always shown to the left of the default sink names
...
SINK_NICKNAMES["alsa_output.pci-0000_00_1f.3.analog-stereo"]="Intern"
SINK_NICKNAMES["bluez_sink.04_52_C7_76_C3_D0.a2dp_sink"]="Bose OE"
...
function output() {
    ...
    if [ "$isMuted" = "yes" ]; then
        ...
        echo "${UNMUTED_COLOR}${volIcon}${curVol}%  ${SINK_ICON}${nickname}"
    fi
}
